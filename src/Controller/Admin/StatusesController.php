...
class StatusesController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Statuses'));
		...
		$this->paginate['Statuses']['page'] 	= $page;
		
		if($sort !== null && $direction !== null){
			$this->paginate['Statuses']['order'] 	= [$sort => $direction];
		}else{
			$this->paginate['Statuses']['order'] 	= $this->defaultOrder;
		}
		...
		if($search !== ''){
			...
			$query = $this->Statuses->find()->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Statuses']['limit'] = $this->config['paginate_limit'];
			$this->paginate['Statuses']['maxLimit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		try {
			$status = $this->Statuses->get((int) $id, contain: ['Cols', 'Tasks']);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'Jeffadmin']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('View the') . ': ' . __('status') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $status->name;
		$this->set(compact('status', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('status') . ' ' . __('record'));
        $status = $this->Statuses->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $status = $this->Statuses->patchEntity($status, $data);
			//dd($status);
			/*
				if(...){
					$status->setErrors('field', __('Message'));
				}
			*/
			//dd($status->getErrors());
            if (!$status->hasErrors() && $this->Statuses->save($status)) {
                //$this->Flash->success(__('The status has been saved.'), ['plugin' => 'Jeffadmin']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'Jeffadmin']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $status->id);
            ...
            //$this->Flash->error(__('The status could not be saved. Please, try again.'), ['plugin' => 'Jeffadmin']);
    ...
    public function edit($id = null)
    {
		...
		try {
			$status = $this->Statuses->get((int) $id, contain: []);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'Jeffadmin']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('Edit the') . ': ' . __('status') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			/*
				if(...){
					$status->setError('field', __('Message'));
				}
			*/
			//dd($status->getErrors());
			if (!$status->hasErrors() && $this->Statuses->save($status)) {
    ...
    public function delete($id = null)
    {
		...
		try {
			$status = $this->Statuses->get((int) $id);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'Jeffadmin']);
			return $this->redirect(['action' => 'index']);
		}
		...
		if ($this->Statuses->delete($status)) {
			...
			//$this->Flash->success(__('The status has been deleted.'), ['plugin' => 'Jeffadmin']);
			$this->Flash->success(__('The has been deleted.'), ['plugin' => 'Jeffadmin']);
		} else {
			//$this->Flash->error(__('The status could not be deleted. Please, try again.'), ['plugin' => 'Jeffadmin']);
