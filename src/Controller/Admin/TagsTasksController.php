...
/**
 * TagsTasks Controller
 *
 * @property \App\Model\Table\TagsTasksTable $TagsTasks
 */
class TagsTasksController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('TagsTasks'));
		...
		$this->paginate['TagsTasks']['page'] 	= $page;
		
		if($sort !== null && $direction !== null){
			$this->paginate['TagsTasks']['order'] 	= [$sort => $direction];
		}else{
			$this->paginate['TagsTasks']['order'] 	= $this->defaultOrder;
		}
		...
		if($search !== ''){
			...
			$query = $this->TagsTasks->find()->contain(['Tags', 'Tasks'])->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['TagsTasks']['limit'] = $this->config['paginate_limit'];
			$this->paginate['TagsTasks']['maxLimit'] = $this->config['paginate_limit'];
			$tagsTasks = $this->paginate($query);
		...
		if(empty($tagsTasks->toArray())){
			return $this->redirect(['action' => 'add']);
		}
		
		$this->set(compact('tagsTasks'));
    }

    /**
     * View method
     *
     * @param string|null $id Tags Task id.
     * @return \Cake\Http\Response|null|void Renders view
    ...
    public function view($id = null)
    {
		...
		try {
			$tagsTask = $this->TagsTasks->get((int) $id, contain: ['Tags', 'Tasks']);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'Jeffadmin']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('View the') . ': ' . __('tagsTask') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $tagsTask->name;
		$this->set(compact('tagsTask', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('tagsTask') . ' ' . __('record'));
        $tagsTask = $this->TagsTasks->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $tagsTask = $this->TagsTasks->patchEntity($tagsTask, $data);
			//dd($tagsTask);
			/*
				if(...){
					$tagsTask->setErrors('field', __('Message'));
				}
			*/
			//dd($tagsTask->getErrors());
            if (!$tagsTask->hasErrors() && $this->TagsTasks->save($tagsTask)) {
                //$this->Flash->success(__('The tags task has been saved.'), ['plugin' => 'Jeffadmin']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'Jeffadmin']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $tagsTask->id);
            ...
            //$this->Flash->error(__('The tags task could not be saved. Please, try again.'), ['plugin' => 'Jeffadmin']);
            $this->Flash->error(__('The save has been not. Please check the datas and try again.'), ['plugin' => 'Jeffadmin']);
        }
        $tags = $this->TagsTasks->Tags->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $tasks = $this->TagsTasks->Tasks->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
        $this->set(compact('tagsTask', 'tags', 'tasks'));
    }
    ...
    public function edit($id = null)
    {
		...
		try {
			$tagsTask = $this->TagsTasks->get((int) $id, contain: []);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'Jeffadmin']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('Edit the') . ': ' . __('tagsTask') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			/*
				if(...){
					$tagsTask->setError('field', __('Message'));
				}
			*/
			//dd($tagsTask->getErrors());
			if (!$tagsTask->hasErrors() && $this->TagsTasks->save($tagsTask)) {
        ...
        $tasks = $this->TagsTasks->Tasks->find('list', conditions: ['visible' => true], limit: 200, order: ['pos' => 'asc', 'name' => 'asc'])->all();
		$name = $tagsTask->name;
        $this->set(compact('tagsTask', 'tags', 'tasks', 'id', 'name'));
    }
    ...
    public function delete($id = null)
    {
		...
		try {
			$tagsTask = $this->TagsTasks->get((int) $id);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'Jeffadmin']);
			return $this->redirect(['action' => 'index']);
		}

		if ($this->TagsTasks->delete($tagsTask)) {
			$this->session->delete('Layout.' . $this->controller . '.LastId');
			//$this->Flash->success(__('The tags task has been deleted.'), ['plugin' => 'Jeffadmin']);
			$this->Flash->success(__('The has been deleted.'), ['plugin' => 'Jeffadmin']);
		} else {
			//$this->Flash->error(__('The tags task could not be deleted. Please, try again.'), ['plugin' => 'Jeffadmin']);
