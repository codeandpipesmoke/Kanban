...
class PositionsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Positions'));
		...
		$this->paginate['Positions']['page'] 	= $page;
		
		if($sort !== null && $direction !== null){
			$this->paginate['Positions']['order'] 	= [$sort => $direction];
		}else{
			$this->paginate['Positions']['order'] 	= $this->defaultOrder;
		}
		...
		if($search !== ''){
			...
			$query = $this->Positions->find()->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Positions']['limit'] = $this->config['paginate_limit'];
			$this->paginate['Positions']['maxLimit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		try {
			$position = $this->Positions->get((string) $id, contain: ['Tasks']);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'Jeffadmin']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('View the') . ': ' . __('position') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $position->name;
		$this->set(compact('position', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('position') . ' ' . __('record'));
        ...
        if ($this->request->is('post')) {
			...
			/*
				if(...){
					$position->setErrors('field', __('Message'));
				}
			*/
			//dd($position->getErrors());
            if (!$position->hasErrors() && $this->Positions->save($position)) {
                //$this->Flash->success(__('The position has been saved.'), ['plugin' => 'Jeffadmin']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'Jeffadmin']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $position->id);
            ...
            //$this->Flash->error(__('The position could not be saved. Please, try again.'), ['plugin' => 'Jeffadmin']);
    ...
    public function edit($id = null)
    {
		...
		try {
			$position = $this->Positions->get((string) $id, contain: []);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'Jeffadmin']);
			return $this->redirect(['action' => 'index']);
		}
		...
		$this->set('title', __('Edit the') . ': ' . __('position') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			/*
				if(...){
					$position->setError('field', __('Message'));
				}
			*/
			//dd($position->getErrors());
			if (!$position->hasErrors() && $this->Positions->save($position)) {
    ...
    public function delete($id = null)
    {
		...
		try {
			$position = $this->Positions->get((int) $id);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'Jeffadmin']);
			return $this->redirect(['action' => 'index']);
		}
		...
		if ($this->Positions->delete($position)) {
			...
			//$this->Flash->success(__('The position has been deleted.'), ['plugin' => 'Jeffadmin']);
			$this->Flash->success(__('The has been deleted.'), ['plugin' => 'Jeffadmin']);
		} else {
			//$this->Flash->error(__('The position could not be deleted. Please, try again.'), ['plugin' => 'Jeffadmin']);
