						...
						<div class="card mb-3">
							...
							<div class="card-header">
								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-eye fa-spin"></i> <?= __('View') ?>: <?= __('Status') ?></h3>
									<?= __('The data marked with <span class="fw-bold text-danger">*</span> must be provided!') ?>
								</div>
								<div class="float-end ms-5">
									<?= $this->Html->link('<span class="btn btn-outline-secondary mt-1 me-1"><i class="fa fa-times"></i></span>',
										["controller" => "Statuses", "action" => "index", "_full" => true],
...
<?php if (!empty($status->cols)) : ?>
												<li><?= $this->Html->link(__('Cols') . '...', ['controller' => 'Cols', 'action' => 'index', 'parent', 'status', $status->id], ['class' => 'dropdown-item']) ?></li>
<?php endif ?>
<?php if (!empty($status->tasks)) : ?>
												<li><?= $this->Html->link(__('Tasks') . '...', ['controller' => 'Tasks', 'action' => 'index', 'parent', 'status', $status->id], ['class' => 'dropdown-item']) ?></li>
<?php endif ?>
											</ul>
										</li>
...
<?php if($config['show_counters']){ ?>
											<div class="row"><!-- counter helper -->
												...
												<div class="col-sm-10 p-1">
													<?= $this->Number->format($status->task_count) ?><!-- 3.b -->
												</div>
											</div>
<?php } ?>
...
<?php if($config['show_counters']){ ?>
											<div class="row"><!-- counter helper -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Col Count') ?>:</label>
												<div class="col-sm-10 p-1">
													<?= $status->col_count === null ? '' : $this->Number->format($status->col_count) ?><!-- 3.a -->
												</div>
											</div>
<?php } ?>
...
<?php } ?>
											...
											<tbody>
												<?php foreach ($status->cols as $cols) : ?>
...
<?php } ?>
<?php if($config['index_show_counters']){ ?>
													<th class="number tag-count"><?= __('Tag Count') ?></th>
<?php } ?>
<?php if($config['index_show_created']){ ?>
													<th class="datetime created"><?= __('Created') ?></th>
<?php } ?>
...
<?php } ?>
<?php if($config['index_show_counters']){ ?>
													<td class="number tag-count" value="<?= $tasks->tag_count ?>"><?= h($tasks->tag_count) ?></td>
<?php } ?>
<?php if($config['index_show_created']){ ?>
