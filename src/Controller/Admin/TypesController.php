...
class TypesController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Types'));
		...
		$this->paginate['Types']['page'] 	= $page;
		
		if($sort !== null && $direction !== null){
			$this->paginate['Types']['order'] 	= [$sort => $direction];
		}else{
			$this->paginate['Types']['order'] 	= $this->defaultOrder;
		}
		...
		if($search !== ''){
			...
			$query = $this->Types->find()->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Types']['limit'] = $this->config['paginate_limit'];
			$this->paginate['Types']['maxLimit'] = $this->config['paginate_limit'];
    ...
    public function view($id = null)
    {
		...
		$this->set('title', __('View the') . ': ' . __('type') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $type->name;
		$this->set(compact('type', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('type') . ' ' . __('record'));
        ...
        if ($this->request->is('post')) {
			...
			/*
				if(...){
					$type->setErrors('field', __('Message'));
				}
			*/
			//dd($type->getErrors());
            if (!$type->hasErrors() && $this->Types->save($type)) {
                //$this->Flash->success(__('The type has been saved.'), ['plugin' => 'Jeffadmin']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'Jeffadmin']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $type->id);
            ...
            //$this->Flash->error(__('The type could not be saved. Please, try again.'), ['plugin' => 'Jeffadmin']);
    ...
    public function edit($id = null)
    {
		...
		$this->set('title', __('Edit the') . ': ' . __('type') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			/*
				if(...){
					$type->setError('field', __('Message'));
				}
			*/
			//dd($type->getErrors());
			if (!$type->hasErrors() && $this->Types->save($type)) {
    ...
    public function delete($id = null)
    {
		...
		if ($this->Types->delete($type)) {
			...
			//$this->Flash->success(__('The type has been deleted.'), ['plugin' => 'Jeffadmin']);
			$this->Flash->success(__('The has been deleted.'), ['plugin' => 'Jeffadmin']);
		} else {
			//$this->Flash->error(__('The type could not be deleted. Please, try again.'), ['plugin' => 'Jeffadmin']);
