 ...
 * @var \App\Model\Entity\TagsTask $tagsTask
 */
						...
						<div class="card mb-3">
							...
							<div class="card-header">
								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-eye fa-spin"></i> <?= __('View') ?>: <?= __('Tags Task') ?></h3>
									<?= __('The data marked with <span class="fw-bold text-danger">*</span> must be provided!') ?>
								</div>
								<div class="float-end ms-5">
									<?= $this->Html->link('<span class="btn btn-outline-secondary mt-1 me-1"><i class="fa fa-times"></i></span>',
										["controller" => "Tags Tasks", "action" => "index", "_full" => true],
...
<?php } ?>
											<div class="row"><!-- 1. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Tag') ?>:</label>
												<div class="col-sm-10 p-1 link">
													<?= $tagsTask->hasValue('tag') ? $this->Html->link($tagsTask->tag->value, ['controller' => 'Tags', 'action' => 'view', $tagsTask->tag->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
											<div class="row"><!-- 1. -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Task') ?>:</label>
												<div class="col-sm-10 p-1 link">
													<?= $tagsTask->hasValue('task') ? $this->Html->link($tagsTask->task->text, ['controller' => 'Tasks', 'action' => 'view', $tagsTask->task->id]) : '' ?><span class="external-link-icon"><i class="fa fa-external-link" aria-hidden="true"></i></span>
												</div>
											</div>
