						...
						<div class="card mb-3">
							...
							<div class="card-header">
								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-eye fa-spin"></i> <?= __('View') ?>: <?= __('Position') ?></h3>
									<?= __('The data marked with <span class="fw-bold text-danger">*</span> must be provided!') ?>
								</div>
								<div class="float-end ms-5">
									<?= $this->Html->link('<span class="btn btn-outline-secondary mt-1 me-1"><i class="fa fa-times"></i></span>',
										["controller" => "Positions", "action" => "index", "_full" => true],
...
<?php if (!empty($position->tasks)) : ?>
												<li><?= $this->Html->link(__('Tasks') . '...', ['controller' => 'Tasks', 'action' => 'index', 'parent', 'position', $position->id], ['class' => 'dropdown-item']) ?></li>
<?php endif ?>
											</ul>
										</li>
...
<?php if($config['show_counters']){ ?>
											<div class="row"><!-- counter helper -->
												<label class="col-sm-2 col-form-label p-1 text-start text-sm-end"><?= __('Task Count') ?>:</label>
												<div class="col-sm-10 p-1">
													<?= $this->Number->format($position->task_count) ?><!-- 3.b -->
												</div>
											</div>
<?php } ?>
...
<?php if (!empty($position->tasks)): ?>
											...
											<button class="nav-link<?= $acticeClass ?>" id="nav-tasks-tab" data-bs-toggle="tab" data-bs-target="#nav-tasks" type="button" role="tab" aria-controls="nav-tasks" aria-selected="true">
												<?= __('Tasks') ?>
											</button>
<?php 	$acticeClass = ""; ?>
<?php endif ?>
...
<?php if (!empty($position->tasks)): ?>
									...
									<div class="tab-pane fade<?= $acticeClass ?> p-0" id="nav-tasks" role="tabpanel" aria-labelledby="nav-tasks-tab" tabindex="0">
...
<?php } ?>
													...
													<th class="string description"><?= __('Description') ?></th>
													<th class="boolean priority"><?= __('Priority') ?></th>
													<th class="boolean deleted"><?= __('Deleted') ?></th>
...
<?php } ?>
											...
											<tbody>
												<?php foreach ($position->tasks as $tasks) : ?>

												<tr>
<?php if($config['index_show_pos']){ ?>
													<td class="number pos" value="<?= $tasks->pos ?>"><?= h($tasks->pos) ?></td>
<?php } ?>
<?php if($config['index_show_id']){ ?>
													<td class="number id" value="<?= $tasks->id ?>"><?= h($tasks->id) ?></td>
<?php } ?>
													<td class="string title" value="<?= $tasks->title ?>"><?= h($tasks->title) ?></td>
													<td class="string description" value="<?= $tasks->description ?>"><?= h($tasks->description) ?></td>
													<td class="boolean priority" value="<?= $tasks->priority ?>"><?= h($tasks->priority) ?></td>
													<td class="boolean deleted" value="<?= $tasks->deleted ?>"><?= h($tasks->deleted) ?></td>
<?php if($config['index_show_visible']){ ?>
													<td class="boolean visible" value="<?= $tasks->visible ?>"><?= h($tasks->visible) ?></td>
<?php } ?>
<?php if($config['index_show_created']){ ?>
													<td class="datetime created" value="<?= $tasks->created ?>"><?= h($tasks->created) ?></td>
<?php } ?>
<?php if($config['index_show_modified']){ ?>
													<td class="datetime modified" value="<?= $tasks->modified ?>"><?= h($tasks->modified) ?></td>
<?php } ?>
													<td class="actions">
														<?= $this->Html->link('<i class="fa fa-eye"></i>', ['controller' => 'Tasks', 'action' => 'view', $tasks->id], ["escape" => false, "role" => "button",  "class" => "btn btn-warning btn-sm", "data-toggle" => "tooltip", "data-placement" => "top", "title" => __('View this item'), "data-original-title" => ""]) ?><!-- view button -->
														<?= $this->Html->link('<i class="fa fa-edit"></i>', ['controller' => 'Tasks', 'action' => 'edit', $tasks->id], ["escape" => false, "role" => "button", "class" => "btn btn-primary btn-sm", "data-toggle" => "tooltip", "data-placement" => "top", "title" => __('Edit this item'), "data-original-title" => ""]) ?><!-- edit button -->
														<?= $this->Form->postLink('<i class="fa fa-times"></i>', ['controller' => 'Tasks', 'action' => 'delete', $tasks->id], ["escape" => false, "role" => "button", "class" => "btn btn-danger btn-sm", "data-toggle" =>"tooltip", "data-placement" => "top", "title" => __('Delete this item'), "data-original-title" => "", "confirm" => __("Are you sure you want to delete # {0}?", $tasks->id)]) ?><!-- delete button -->
													</td>
												</tr>
												<?php endforeach ?>
