						...
						<div class="card mb-3">
							...
							<div class="card-header">
								<div class="float-start">
									<h3><i id="card-icon" class="fa fa-eye fa-spin"></i> <?= __('View') ?>: <?= __('View') ?></h3>
									<?= __('The data marked with <span class="fw-bold text-danger">*</span> must be provided!') ?>
								</div>
								<div class="float-end ms-5">
									<?= $this->Html->link('<span class="btn btn-outline-secondary mt-1 me-1"><i class="fa fa-times"></i></span>',
										["controller" => "Views", "action" => "index", "_full" => true],
...
<?php if (!empty($view->cols)) : ?>
												<li><?= $this->Html->link(__('Cols') . '...', ['controller' => 'Cols', 'action' => 'index', 'parent', 'view', $view->id], ['class' => 'dropdown-item']) ?></li>
<?php endif ?>
											</ul>
										</li>
...
<?php if($config['show_counters']){ ?>
											<div class="row"><!-- counter helper -->
												...
												<div class="col-sm-10 p-1">
													<?= $view->col_count === null ? '' : $this->Number->format($view->col_count) ?><!-- 3.a -->
												</div>
											</div>
<?php } ?>
