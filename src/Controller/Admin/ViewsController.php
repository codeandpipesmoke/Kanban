 ...
 * Views Controller
 *
 * @property \App\Model\Table\ViewsTable $Views
 */
class ViewsController extends AppController
{
    ...
    public function index($clearFilter = null)
    {
		...
		$this->set('title', __('Browse the') . ': ' . __('Views'));
		...
		$this->paginate['Views']['page'] 	= $page;
		
		if($sort !== null && $direction !== null){
			$this->paginate['Views']['order'] 	= [$sort => $direction];
		}else{
			$this->paginate['Views']['order'] 	= $this->defaultOrder;
		}
		...
		if($search !== ''){
			...
			$query = $this->Views->find()->where($conditions);
		}
		// ############################# /.QUERY ###########################################
		...
		// ############################# PAGINATE ############################################
		try {
			$this->paginate['Views']['limit'] = $this->config['paginate_limit'];
			$this->paginate['Views']['maxLimit'] = $this->config['paginate_limit'];
			$views = $this->paginate($query);
    ...
    public function view($id = null)
    {
		...
		try {
			$view = $this->Views->get((int) $id, contain: ['Cols']);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'Jeffadmin']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('View the') . ': ' . __('view') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		$name = $view->name;
		$this->set(compact('view', 'id', 'name'));
    }
    ...
    public function add()
    {
		...
		$this->set('title', __('Add new') . ': ' . __('view') . ' ' . __('record'));
        $view = $this->Views->newEmptyEntity();
        if ($this->request->is('post')) {
			$data = $this->request->getData();
			//debug($data);
            $view = $this->Views->patchEntity($view, $data);
			//dd($view);
			/*
				if(...){
					$view->setErrors('field', __('Message'));
				}
			*/
			//dd($view->getErrors());
            if (!$view->hasErrors() && $this->Views->save($view)) {
                //$this->Flash->success(__('The view has been saved.'), ['plugin' => 'Jeffadmin']);
                $this->Flash->success(__('The save has been: OK'), ['plugin' => 'Jeffadmin']);
				$this->session->write('Layout.' . $this->controller . '.LastId', $view->id);
            ...
            //$this->Flash->error(__('The view could not be saved. Please, try again.'), ['plugin' => 'Jeffadmin']);
    ...
    public function edit($id = null)
    {
		...
		try {
			$view = $this->Views->get((int) $id, contain: []);
		} catch (\Cake\Datasource\Exception\RecordNotFoundException $exeption) {
			$this->Flash->warning(__($exeption->getMessage()), ['plugin' => 'Jeffadmin']);
			return $this->redirect(['action' => 'index']);
		}

		$this->set('title', __('Edit the') . ': ' . __('view') . ' ' . __('record'));
		$this->session->write('Layout.' . $this->controller . '.LastId', $id);
		...
		if ($this->request->is(['patch', 'post', 'put'])) {
			...
			/*
				if(...){
					$view->setError('field', __('Message'));
				}
			*/
			//dd($view->getErrors());
			if (!$view->hasErrors() && $this->Views->save($view)) {
    ...
    public function delete($id = null)
    {
		...
		if ($this->Views->delete($view)) {
			...
			//$this->Flash->success(__('The view has been deleted.'), ['plugin' => 'Jeffadmin']);
			$this->Flash->success(__('The has been deleted.'), ['plugin' => 'Jeffadmin']);
		} else {
			//$this->Flash->error(__('The view could not be deleted. Please, try again.'), ['plugin' => 'Jeffadmin']);
